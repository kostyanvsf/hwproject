...
class Fulfill extends React.Component {
  ...
  UserAction(orderId) {
    ...
    axios
      .put(
        `${CONFIG.APIBaseUrl}/listingorder/fulfill/${orderId}`,
        {},
        { headers }
      )
      .then(res => {
  ...
  componentDidMount() {
    ...
    axios
      .get(`${CONFIG.APIBaseUrl}/listingorder/allOrders`, { headers })
      .then(res => {
        // console.log(res);
                ...
                <Grid item key={order.orderId} sm={6} md={4} lg={3}>
                  <Card className={classes.card}>
                    ...
                    <CardContent className={classes.cardContent}>
                      ...
                      {order.listings.length > 0 ? (
                        <Typography>{order.listings[0].listingName}</Typography>
                      ) : (
                    ...
                    <CardActions>
                      ...
                      {order.isFulfilled !== "No" ? (
                        <button
                          disabled
                          type="submit"
                          onClick={() => this.UserAction(order.orderId)}
                        >
                          Fulfilled
                        </button>
                      ) : (
